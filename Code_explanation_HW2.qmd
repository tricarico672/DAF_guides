...
```{r}
hours <- read_csv("Homework2/hours.dat")
...
it is concluded that a box-cox transformation with lambda = -0.20... improves the fit to normality of the data as shown by the quantile-quantile plot and the histogram.
...
1.  you estimate the auto correlation function with ACF(dataset, column_in_dataset) and assign it to a variable (autocorr in my example above)
